...
class HeartRateCreate(BaseModel):
    ...
    def validate_context(cls, v):
        valid_contexts = ['resting', 'active', 'post-coffee',
                          'exercise', 'sleeping', 'panic', 'stressed']
...
def get_current_heartrate(db: Session = Depends(get_db)):
    ...
    latest = db.query(HeartRateLog).order_by(
        HeartRateLog.timestamp.desc()).first()
    if not latest:
...
def get_heartrate_stats(days: int = Query(30, ge=1, le=365), db: Session = Depends(get_db)):
    ...
    logs = db.query(HeartRateLog).filter(
        HeartRateLog.timestamp >= cutoff).all()
...
def get_caffeine_correlation(hours_after: int = Query(2, ge=1, le=24), db: Session = Depends(get_db)):
    ...
    for coffee in coffee_logs:
        ...
        start_time = coffee.timestamp + \
            timedelta(minutes=15)  # Give 15 min for absorption
        ...
        if hr_readings:
            ...
            baseline_hr = statistics.mean(
                [r.bpm for r in baseline_readings]) if baseline_readings else None
    ...
    hr_increases = [c["hr_increase"]
                    for c in correlations if c["hr_increase"] is not None]
